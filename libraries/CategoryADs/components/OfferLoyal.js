...
export class CustomerClub extends Component {
    ...
    buy_from_loyality_club = (product_id, shop_id) => {
        fetch(L_URL + 'addorder', {
            method: 'post', headers: { 'Authorization': this.state.loyality_token, 'content-type': 'application/json' },
            body: JSON.stringify({ product_id: product_id, number: 1 })
        }).then(response => {
    ...
    render() {
        return (
            <View style={{ flex: 1, marginTop: 10 }}>
                {!this.state.OfferLoyalData.length == 0 ?
                    <FlatList
                        ...
                        } /> : !this.state.loaded && <Text style={{ fontFamily: 'IRANSans(FaNum)', fontSize: 25, textAlign: 'center', marginTop: 100, marginHorizontal: 20 ,color: '#573c65'}}>آگهی تخفیفی برای این دسته در تخفیف‌های باشگاه مشتریان موجود نیست!</Text>}
